...
const ClassicTemplate = ({ data }: { data: ResumeData }) => (
  <div style={{ fontFamily: 'Georgia, serif', fontSize: 11 }} className="px-6 py-5">
    <div className="text-center border-b-2 border-gray-800 pb-4 mb-4">
      ...
      <div className="flex justify-center flex-wrap gap-2 mt-1 text-xs text-gray-600">
        {[data.personalInfo.email, data.personalInfo.phone, data.personalInfo.location].filter(Boolean).join('  |  ')}
      </div>
      {(data.personalInfo.linkedin || data.personalInfo.portfolio) && (
        <div className="text-xs text-gray-500 mt-0.5">{[data.personalInfo.linkedin, data.personalInfo.portfolio].filter(Boolean).join('  |  ')}</div>
      )}
    </div>
          ...
          {data.experience.map(exp => (
            <div key={exp.id} className="mb-3">
              <div className="flex justify-between items-baseline">
                <span className="font-bold text-xs text-gray-900">{exp.title}, {exp.company}</span>
        ...
        <div><SectionLabel label="Education" template="classic" />
          {data.education.map(edu => (
            <div key={edu.id} className="mb-2 flex justify-between items-baseline">
              ...
              <span className="text-xs text-gray-500 italic">{edu.duration}{edu.grade ? ` | ${edu.grade}` : ''}</span>
            </div>
          ))}
        </div>
      )}
