    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6">
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xl sm:text-2xl font-bold text-green-700">{verified.length}</p>
            <p className="text-xs sm:text-sm text-green-600">Verified</p>
          </div>
        </div>
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
          <Clock className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-600 flex-shrink-0" />
          <div>
            <p className="text-xl sm:text-2xl font-bold text-yellow-700">{pending.length}</p>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xl sm:text-2xl font-bold text-blue-700">{candidates.length}</p>
              ...
              {DAYS.map(day => (
                <div key={day} className="text-center">
                  ...
                  <input
                    ...
                    onChange={e => updateHours(id, day, Number(e.target.value))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  min={0}
                  value={b.rate}
                  onChange={e => setBillingData(prev => ({ ...prev, [id]: { ...prev[id], rate: Number(e.target.value) } }))}
              ...
              <div>
                ...
                <input
                  ...
                  min={0}
                  value={b.hours}
                  onChange={e => setBillingData(prev => ({ ...prev, [id]: { ...prev[id], hours: Number(e.target.value) } }))}
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
