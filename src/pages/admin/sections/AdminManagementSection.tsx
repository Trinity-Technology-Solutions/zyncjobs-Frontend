...
export default function AdminManagementSection({ 
  ...
  const loadAdmins = useCallback(async () => {
    ...
    setError('');
    try {
      const [adminRes, superRes, recruiterRes] = await Promise.allSettled([
        ...
        authFetch(`${API_ENDPOINTS.ADMIN_USERS}?role=recruiter`, {}, onUnauthorized),
      ]);
      ...
      const recruiters: Admin[] = recruiterRes.status === 'fulfilled' ? (recruiterRes.value.users ?? recruiterRes.value.data ?? recruiterRes.value ?? []) : [];
      const seen = new Set<string>();
      const merged = [...admins, ...superAdmins, ...recruiters].filter(u => {
        const id = (u._id || u.id) ?? '';
...
const [confirmState, setConfirmState] = useState<{ open: boolean; message: string; onConfirm: () => void }>({ open: false, message: '', onConfirm: () => {} });
  ...
  const deleteAdmin = (adminId: string, adminName: string) => {
    setConfirmState({
      ...
      onConfirm: () => execDeleteAdmin(adminId, adminName)
    });
  };

  const execDeleteAdmin = async (adminId: string, adminName: string) => {
                      ...
                      <tr key={adminId} className="border-b border-gray-800 hover:bg-gray-800/40 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          {isCurrentUser ? (
                            ...
                            <AutocompleteCombobox
                              value={admin.role}
                              onChange={(val) => updateAdminRole(adminId!, val as 'admin' | 'super_admin' | 'recruiter')}
